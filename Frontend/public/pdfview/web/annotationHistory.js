...
const addHistory = async function (id, type, username, date, page, typeString, formFieldName) {
    if (id !== 0) {
        if (!showHistoryBar.querySelector(`#history${id}`)) {
            ...
            replyInput.setAttribute('autocomplete', false);
...
const saveDoc = async function () {
    ...
    if(initialId && isDraft){
        ...
        fetch(`${BASE_URL}/history/` + initialId, {
            ...
            .then(() => {
                $("#modal-success #open-documents").removeClass("hidden");
                $("#modal-success").show();
            })
            .catch((error) => {
        ...
        .then(data => {
            ...
            $("#modal-success #open-documents").removeClass("hidden");
            $("#modal-success #refresh-page").removeClass("hidden");
            $("#modal-success").show();
        })
        .catch((error) => {
            console.error('Error:', error);
...
const drawHistory = function(data) {
